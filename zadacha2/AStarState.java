...
public class AStarState
{
    ...
    public Waypoint getMinOpenWaypoint()
    {
		...
		point=openpoints.values().toArray(point);
		Waypoint minp = point[0];
		double cost, min = minp.getTotalCost();
        for(int i=1;i<point.length;i++)
		{
			cost = point[i].getTotalCost();
			if(cost<min)
			{
				minp=point[i];
				min=cost;
    ...
    {
        Waypoint point = openpoints.get(newWP.getLocation());
    ...
    {
        Waypoint point = openpoints.remove(loc);
        if(point != null)
        {
            closepoints.put(loc, point);
        }
    }
